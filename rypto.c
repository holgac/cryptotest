...
unsigned char rijndael_s_box[256] = 
{
...
unsigned char rijndael_invs_box[256] = 
{
...
static inline unsigned char rijndael_div2(unsigned char d)
{
	return (d >> 1) ^ (0x8d * (d&1));
}
static inline unsigned char rijndael_mult2(unsigned char d)
{
	return (d << 1) ^ (0x11b & -(d>>7));
}

void calc_new_rk(unsigned char *rk, unsigned char *rc)
{
	size_t i;
	unsigned char old[16];
	memcpy(old, rk, 16);
	rk[0] = rijndael_s_box[old[13]] ^ old[0] ^ *rc;
	rk[1] = rijndael_s_box[old[14]] ^ old[1];
	rk[2] = rijndael_s_box[old[15]] ^ old[2];
	rk[3] = rijndael_s_box[old[12]] ^ old[3];
	for(i=4; i<16; ++i)
		rk[i] = rk[i-4] ^ old[i];
	*rc = rijndael_mult2(*rc);
}

void calc_prev_rk(unsigned char *rk, unsigned char *rc)
{
	size_t i;
	unsigned char old[16];
	memcpy(old, rk, 16);
	*rc = rijndael_div2(*rc);
	for(i=4; i<16; ++i)
		rk[i] = old[i-4] ^ old[i];
	rk[0] = rijndael_s_box[rk[13]] ^ old[0] ^ *rc;
	rk[1] = rijndael_s_box[rk[14]] ^ old[1];
	rk[2] = rijndael_s_box[rk[15]] ^ old[2];
	rk[3] = rijndael_s_box[rk[12]] ^ old[3];
}

static void rijndael_add_round_key(unsigned char *data, unsigned char *rk)
{
	size_t i;
	for(i=0; i<16; ++i)
		data[i] ^= rk[i];
...
static void rijndael_sub_byte(unsigned char *data)
{
	size_t i;
	for(i=0; i<16; ++i)
		data[i] = rijndael_s_box[data[i]];
}
static void rijndael_shift_row(unsigned char *data)
{
	unsigned char tmp;
	tmp = data[1];
	data[1] = data[5];
...
static void rijndael_mix_column(unsigned char *data)
{
	unsigned char old[16];
	size_t i;
	memcpy(old, data, 16);
	for(i=0; i<16; i+=4) {
		data[i] = rijndael_mult2(old[i]) ^ rijndael_mult2(old[i+1]) ^ old[i+1] ^ old[i+2] ^ old[i+3];
		data[i+1] = old[i] ^ rijndael_mult2(old[i+1]) ^ rijndael_mult2(old[i+2]) ^ old[i+2] ^ old[i+3];
		data[i+2] = old[i] ^ old[i+1] ^ rijndael_mult2(old[i+2]) ^ rijndael_mult2(old[i+3]) ^ old[i+3];
		data[i+3] = rijndael_mult2(old[i]) ^ old[i] ^ old[i+1] ^ old[i+2] ^ rijndael_mult2(old[i+3]);
	}
}
...
{
	size_t nr = 9, r=0;
	unsigned char rk[16], rc=1;
	size_t num_blocks, i;

	memcpy(rk, key, 16);
	num_blocks = datalen / 16;
	memcpy(out, data, datalen);
	/*
	 * round 0
	 */
	for(i=0; i<num_blocks; ++i)
		rijndael_add_round_key(out+i*16, rk);

	calc_new_rk(rk, &rc);
	for(r=0; r<nr; ++r) {
		for(i=0; i<num_blocks; ++i) {
			rijndael_sub_byte(out+i*16);
			rijndael_shift_row(out+i*16);
			rijndael_mix_column(out+i*16);
			rijndael_add_round_key(out+i*16, rk);
		}
		calc_new_rk(rk, &rc);
	}
...
static void rijndael_invsub_byte(unsigned char *data)
{
	size_t i;
	for(i=0; i<16; ++i)
		data[i] = rijndael_invs_box[data[i]];
}
static void rijndael_invshift_row(unsigned char *data)
{
	unsigned char tmp;
	tmp = data[9];
	data[9] = data[5];
...
static unsigned char rijndael_mult9[] = {
...
static unsigned char rijndael_mult11[] = {
...
static unsigned char rijndael_mult13[] = {
...
static unsigned char rijndael_mult14[] = {
...
static void rijndael_invmix_column(unsigned char *data)
{
	unsigned char old[16];
	size_t i;
	memcpy(old, data, 16);
	for(i=0; i<16; i+=4) {
		data[i] = rijndael_mult14[old[i]] ^ rijndael_mult11[old[i+1]] ^ rijndael_mult13[old[i+2]] ^ rijndael_mult9[old[i+3]];
		data[i+1] = rijndael_mult9[old[i]] ^ rijndael_mult14[old[i+1]] ^ rijndael_mult11[old[i+2]] ^ rijndael_mult13[old[i+3]];
		data[i+2] = rijndael_mult13[old[i]] ^ rijndael_mult9[old[i+1]] ^ rijndael_mult14[old[i+2]] ^ rijndael_mult11[old[i+3]];
		data[i+3] = rijndael_mult11[old[i]] ^ rijndael_mult13[old[i+1]] ^ rijndael_mult9[old[i+2]] ^ rijndael_mult14[old[i+3]];
	}
}
		...
		unsigned char *out)
{
	...
	for(r=0; r<nr; ++r) {
		for(i=0; i<num_blocks; ++i) {
			rijndael_invshift_row(out+i*16);
			rijndael_invsub_byte(out+i*16);
			rijndael_add_round_key(out+i*16, rk);
			rijndael_invmix_column(out+i*16);
		}
		calc_prev_rk(rk, &rc);
	}
	for(i=0; i<num_blocks; ++i) {
